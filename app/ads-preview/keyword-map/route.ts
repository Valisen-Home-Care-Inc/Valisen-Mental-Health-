...
export function GET() {
  ...
  const rows = [
    ["Ad group", "Keyword", "Match type", "Live final URL", "Preview path"],
  ...
  return new Response("\uFEFF" + rows.map((row) => row.map(escape).join(",")).join("\r\n"), {
    headers: { "Content-Type": "text/csv; charset=utf-8", "Content-Disposition": 'attachment; filename="valisen-google-ads-keyword-urls.csv"', "Cache-Control": "no-store", "X-Robots-Tag": "noindex, nofollow" },
  });
}
