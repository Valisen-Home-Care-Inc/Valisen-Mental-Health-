...
export function parseQuizConsultationSlot(date: unknown, time: unknown, now = new Date(), pool: readonly ConsultationTherapist[] = ALL_CONSULTATION_THERAPISTS): QuizConsultationSlot | null {
  ...
  const slot = getAvailableTimeSlotsForDate(date, pool).find((entry) => entry.time === time);
